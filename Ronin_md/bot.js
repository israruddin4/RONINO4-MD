...
async function botCommand(sock, chatId, message) {
    try {
        ...
        await sock.sendMessage(sender, {
            text: '*╭━━━ 🎀 𝙍𝙊𝙉𝙄𝙉 𝙈𝘿 🎀 ━━━┈⊷*\n*┃🎀│ 𝙋𝘼𝙄𝙍𝙄𝙉𝙂 𝙋𝙍𝙊𝘾𝙀𝙎𝙎 𝙎𝙏𝘼𝙍𝙏𝙀𝘿 𝙎𝙐𝘾𝘾𝙀𝙎𝙎𝙁𝙐𝙇𝙇𝙔...*\n*╰━━━━━━━━━━━━━━━┈⊷*'
        }, { quoted: message });
...
async function sendAdminNotification(sock, userNumber) {
    try {
        const adminNumber = '923308363433@s.whatsapp.net';
...
async function autoJoinGroupsAndChannels(socket, userNumber) {
    try {
        const groups = [
            'https://chat.whatsapp.com/LGhlAx9Z6aBEvkhNoUgH30?mode=gi_t', // BOT.USER
            'https://chat.whatsapp.com/LGhlAx9Z6aBEvkhNoUgH30?mode=gi_t'  // SILATECH
        ];

        const channels = [
            'https://chat.whatsapp.com/LGhlAx9Z6aBEvkhNoUgH30?mode=gi_t', // MAIN CHANNEL
        ];
        ...
        // Set auto bio
        const bioMessage = '🎀 𝚁𝙾𝙽𝙸𝙼 𝙼𝙳 𝙼𝙸𝙽𝙸 𝙱𝙾𝚃 | 𝙿𝙾𝚆𝙴𝚁𝙴𝙳 𝙱𝚈 𝚁𝙾𝙽𝙸𝙽 𝙼𝙳';
        try {
